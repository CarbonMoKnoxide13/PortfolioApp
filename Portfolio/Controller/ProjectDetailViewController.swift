...
class ProjectDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func nextSlide(_ sender: UIButton) {
        if (currentScreen < project.screens.count - 1) {
            ...
            codeTableView.scroll(to: .top, animated: true)
        }
    }
    ...
    func setupConstraints() {
        ...
        view.addConstraint(NSLayoutConstraint(item: codeTableView!, attribute: .top, relatedBy: .equal, toItem: screenshotImageView, attribute: .bottom, multiplier: 1, constant: 16))
        ...
        //bottom
        view.addConstraint(NSLayoutConstraint(item: screenshotImageView!, attribute: .bottom, relatedBy: .equal, toItem: view, attribute: .centerY, multiplier: 1, constant: 16))
