//
//  TarotScrollFlowLayout.swift
//  Portfolio
//
//  Created by Aramis Knox on 10/7/19.
...
class TarotScrollLayout : UICollectionViewFlowLayout {
    
    open var springHardness: CGFloat = 50 //15
    ...
    override open func prepare() {
        ...
        //The visible rect is the size of the frame of the collection view cell
        let visibleRect = CGRect(origin: collectionView.bounds.origin,
                                 size: collectionView.frame.size).insetBy(dx: 0, dy: offset)
    ...
    private func addBehaviors(for items: [UICollectionViewLayoutAttributes]) {
        ...
        items.forEach { item in
            ...
            springBehaviour.damping = 3.0 //0.8 for extreme bounciness
            springBehaviour.frequency = 2.0
    ...
    private func getUpdatedBehaviorItemCenter(behavior: UIAttachmentBehavior,
        ...
        let xDistanceFromTouch = fabs(touchLocation.x)
